...
module.exports = function (model, collection, docs) {
  ...
  // This is a temp fix for the momment, need to work at it
  const [namespace, entityTypeName] = entitySet.entityType.split('.')
  // Get the entity type from the schema
  const correctSchema = model.schemas.find(s => s?.namespace === namespace && s.entityTypes[entityTypeName])
  const entityType = correctSchema.entityTypes[entityTypeName]
